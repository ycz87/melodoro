...
const outDir = process.argv[2] ?? 'artifacts/issue-132/baseline-current';

const viewports = [
  { name: '390x844', width: 390, height: 844, isMobile: true, deviceScaleFactor: 3 },
  { name: '360x800', width: 360, height: 800, isMobile: true, deviceScaleFactor: 3 },
  { name: '1440x900', width: 1440, height: 900, isMobile: false, deviceScaleFactor: 1 },
];
...
async function captureMetrics(page) {
  return page.evaluate(() => {
    ...
    const cottage = document.querySelector('[data-testid="farm-v2-cottage"]');
    const tiles = Array.from(document.querySelectorAll('[data-testid="farm-plot-board-v2"] [data-slot-state]'));
    const firstRow = tiles.slice(0, 3).map((tile) => tile.getBoundingClientRect());
    ...
    return {
      ...
      cottageRect: cottage?.getBoundingClientRect() ?? null,
      firstRow,
      lastRow,
      badgeToFenceGap: weatherBadge && fence ? fence.getBoundingClientRect().top - weatherBadge.getBoundingClientRect().bottom : null,
      fenceToBoardGap: fence && board ? board.getBoundingClientRect().top - fence.getBoundingClientRect().bottom : null,
      sceneScrollBottomGap: scene ? scene.getBoundingClientRect().bottom - window.innerHeight : null,
    };
  });
}

async function runClickChecks(page, outDir, viewportName) {
  const buttons = page.locator('[data-testid="farm-plot-board-v2"] [role="button"]');

  await buttons.nth(0).click();
  const emptyModalVisible = await page.getByRole('heading', { name: '选择种子' }).isVisible();
  await page.screenshot({ path: path.join(outDir, `click-empty-${viewportName}.png`), fullPage: true });
  await page.getByRole('button', { name: '取消' }).click();

  await buttons.nth(4).click();
  const growingCardVisible = await page.locator('[data-testid="farm-v2-growth-info-card"]').isVisible();
  await page.screenshot({ path: path.join(outDir, `click-growing-${viewportName}.png`), fullPage: true });
  await page.getByLabel('Close growth tooltip').click();

  await buttons.nth(8).click();
  await page.waitForTimeout(250);
  const matureBecameEmpty = await page.evaluate(() => {
    const slots = Array.from(document.querySelectorAll('[data-testid="farm-plot-board-v2"] [data-slot-state]'));
    return slots.at(-1)?.getAttribute('data-slot-state') === 'empty';
  });
  await page.screenshot({ path: path.join(outDir, `click-mature-${viewportName}.png`), fullPage: true });

  return { emptyModalVisible, growingCardVisible, matureBecameEmpty };
}
...
try {
  for (const viewport of viewports) {
    ...
    const baseName = `farm-${viewport.name}`;
    await page.screenshot({ path: path.join(outDir, `${baseName}.png`), fullPage: true });
    const clicks = viewport.isMobile ? await runClickChecks(page, outDir, viewport.name) : null;
    await fs.writeFile(path.join(outDir, `${baseName}.json`), JSON.stringify({ metrics, clicks }, null, 2));
