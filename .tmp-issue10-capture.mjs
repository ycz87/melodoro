...
const outDir = path.resolve('artifacts/issue-10/baseline');
...
async function captureViewport(browser, viewport) {
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:4173/?farmReview=1', { waitUntil: 'networkidle' });
    ...
    const metrics = await page.evaluate(() => {
      ...
      const farmPage = document.querySelector('.farm-page');
      const farmScene = board?.closest('[style]');
      ...
      return {
        ...
        farmPageRect: farmPage ? (() => {
          const rect = farmPage.getBoundingClientRect();
          return {
            left: rect.left,
            top: rect.top,
            right: rect.right,
        ...
        sceneRect: farmScene ? (() => {
          const rect = farmScene.getBoundingClientRect();
          return {
            left: rect.left,
            top: rect.top,
            right: rect.right,
