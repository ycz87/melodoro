...
export function SimpleFarmGrid({
  ...
  const plotScale = compactMode
    ? (isCompactMobile ? 1.24 : isMobile ? 1.28 : 1.38)
    : 1;
  ...
  const sceneTopPadding = compactMode
    ? (isCompactMobile
      ? Math.round(effectivePlotSize * 1.46)
      : isMobile
        ? Math.round(effectivePlotSize * 1.4)
  ...
  const slotOffsetScale = compactMode
    ? (isCompactMobile ? 0.94 : isMobile ? 0.97 : 1)
        ...
        >
          <div
            ...
            style={{
              ...
              gridAutoRows: `${Math.round(effectivePlotSize * (compactMode ? (isMobile ? 0.76 : 0.66) : 0.56))}px`,
            }}
          >
